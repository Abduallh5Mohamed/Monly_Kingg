...
router.post("/:id/credentials", validateObjectId(), txController.submitCredentials);

// Buyer: confirm receipt → release funds to seller
router.post("/:id/confirm", validateObjectId(), txController.confirmReceived);

// Buyer: open a dispute
router.post("/:id/dispute", validateObjectId(), txController.openDispute);

// Admin: resolve dispute (refund or release)
router.post("/:id/resolve", validateObjectId(), requireAdminOrMod, requirePermission("orders"), txController.adminResolveDispute);
