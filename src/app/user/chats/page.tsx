...
export default function SupportPage() {
  ...
  useEffect(() => {
    ...
    newSocket.on('connect', () => {
      ...
      // Rejoin current chat on reconnect
      if (selectedChatRef.current) {
        newSocket.emit('join_chat', selectedChatRef.current._id);
      }
    });
    ...
    // Presence events (no selectedChat dependency)
    ...
    // Typing events (no selectedChat dependency)
    ...
    // Chat list update from socket (supplement API load)
  ...
  // ─── Message-specific socket events (fresh selectedChat/currentUserId via refs) ──
  useEffect(() => {
    if (!socket) return;

    const onNewMessage = ({ chatId, message }: { chatId: string; message: Message }) => {
      // Update the chat list preview + unread badge for background chats
      ...
      // Only append to messages view if this chat is currently open
      ...
      // Mark delivered / read since we're looking at it
    ...
    const onMessageSent = ({ chatId, message, tempId }: { chatId: string; message: Message; tempId?: string }) => {
      if (selectedChatRef.current?._id !== chatId) return;
      setMessages(prev => {
        // Remove the optimistic placeholder by its exact tempId (most reliable),
        // falling back to removing all unconfirmed temp messages for this content
        const withoutTemp = tempId
          ? prev.filter(m => m._id !== tempId)
          : prev.filter(m => !m._id.startsWith('temp-') || m.content !== message.content);
        // Add real message if not already present (dedup)
    ...
    const onMessagesRead = ({ chatId }: { chatId: string }) => {
      ...
      setChats(prev => prev.map(chat =>
        chat._id === chatId
          ? { ...chat, unreadCount: 0, lastMessage: chat.lastMessage ? { ...chat.lastMessage, read: true } : undefined }
          : chat
      ));
    };
  ...
  }, [socket]); // only re-runs when socket instance changes; refs keep other values fresh
  ...
  useEffect(() => {
    ...
    // Clear messages immediately so the old chat's messages don't flash
    setMessages([]);
    ...
    // Load via API (always works, socket is just real-time on top)
    loadMessages(selectedChat._id);

    // Join chat room and mark read via socket
    if (socket?.connected) {
      socket.emit('join_chat', selectedChat._id);
      socket.emit('mark_read', { chatId: selectedChat._id });
    }
  }, [selectedChat]); // ← only selectedChat; socket join handled separately

  // When socket connects/reconnects, rejoin the current chat room
  useEffect(() => {
    ...
    try {
      ...
      if (socket && socket.connected) {
        socket.emit('send_message', {
          ...
          content: messageContent,
          type: 'text',
          tempId: tempMessage._id  // echo back so server can confirm exact placeholder
        });
        ...
        // Replace temp message with the real confirmed message
