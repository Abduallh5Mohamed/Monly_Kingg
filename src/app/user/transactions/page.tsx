      ...
      <div className="bg-[#060811]">
        ...
        <div className="relative max-w-3xl mx-auto px-4 py-5 sm:py-8">
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <h1 className="text-2xl font-bold text-white">{tr('معاملاتي', 'My Transactions')}</h1>
          </div>
          ...
          <div className="flex gap-2 mb-6">
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl font-medium text-sm transition-all border ${tab === 'buyer'
            ...
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl font-medium text-sm transition-all border ${tab === 'seller'
          ...
          {loading ? (
            ...
            <div className="space-y-3">
              {txs.map(tx => {
                const statusMeta = STATUS_LABEL[tx.status];
                const label = statusMeta ? (language === 'ar' ? statusMeta.ar : statusMeta.en) : tx.status;
                    ...
                    <div className={`bg-[#0a0d16]/80 border rounded-2xl p-4 hover:border-cyan-500/30 transition-all cursor-pointer ${needsAction ? 'border-cyan-500/40 ring-1 ring-cyan-500/20' : 'border-white/[0.06]'
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex-1 min-w-0">
                          <p className="text-white font-medium truncate">{tx.listing?.title || tr('إعلان', 'Listing')}</p>
                          <p className="text-white/40 text-xs mt-0.5">
                            {tab === 'buyer'
                              ? `${tr('البائع', 'Seller')}: ${tx.seller?.username}`
                              : `${tr('المشتري', 'Buyer')}: ${tx.buyer?.username}`} · {new Date(tx.createdAt).toLocaleDateString(language === 'ar' ? 'ar-EG' : 'en-US')}
                          </p>
                        </div>
                      ...
                      {tx.status === 'waiting_buyer' && tx.autoConfirmAt && (
                        <p className="mt-2 text-white/40 text-xs">
                          {tr('تأكيد تلقائي', 'Auto confirm')}: {new Date(tx.autoConfirmAt).toLocaleString(language === 'ar' ? 'ar-EG' : 'en-US')}
                        </p>
                      )}
                    </div>
                  </Link>
