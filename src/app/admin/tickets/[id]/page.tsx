...
import {
  ArrowLeft, Send, Shield, User, Store, Clock, CheckCircle2,
  AlertCircle, X, ChevronDown, MessageSquare, Paperclip, FileText, Image as ImageIcon
} from 'lucide-react';
...
export default function AdminTicketDetailPage() {
  ...
  // Poll every 8s for live chat feel
  useEffect(() => {
    if (!user || !ticket || ticket.status === 'closed') return;
    const interval = setInterval(fetchTicket, 8000);
  ...
  const handlePriorityChange = async (newPriority: string) => {
    ...
    try {
      const csrfToken = await ensureCsrfToken();
      const res = await fetch(`/api/v1/tickets/${params.id}/priority`, {
        method: 'PATCH',
        credentials: 'include',
        headers: { 'Content-Type': 'application/json', ...(csrfToken ? { 'X-XSRF-TOKEN': csrfToken } : {}) },
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-120px)]">
      {/* Ticket Info Header */}
      <div className="bg-[#0f1117]/80 border border-white/[0.06] rounded-xl p-4 mb-4 shrink-0">
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            <div className="flex items-center gap-2 mb-2 flex-wrap">
              <button onClick={() => router.push('/admin/tickets')} className="p-1.5 rounded-lg bg-white/[0.04] hover:bg-white/[0.08] text-white/70 hover:text-white transition-colors">
            ...
            <div className="flex items-center gap-3 mt-2 flex-wrap">
              <div className="flex items-center gap-1.5">
                {ticket.userRole === 'seller' ? <Store className="w-3.5 h-3.5 text-emerald-400" /> : <User className="w-3.5 h-3.5 text-cyan-400" />}
                <span className="text-white/70 text-sm">{ticket.user.username}</span>
              ...
              {ticket.user.email && <span className="text-white/30 text-xs">{ticket.user.email}</span>}
              <span className="text-white/20">·</span>
              <span className="text-white/30 text-xs">{CATEGORY_MAP[ticket.category] || ticket.category}</span>
              <span className="text-white/20">·</span>
              <span className="text-white/30 text-xs">{new Date(ticket.createdAt).toLocaleString()}</span>
            </div>
          </div>
          ...
          <div className="shrink-0 flex flex-col gap-2 items-end">
            ...
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                disabled={updatingPriority || ticket.status === 'closed'}
                className={`px-2.5 py-1 rounded-lg text-xs font-medium border bg-transparent cursor-pointer focus:outline-none ${ticket.priority === 'high' ? 'border-red-500/30 text-red-400' :
            ...
            {ticket.status !== 'closed' && (
              ...
              >
                {closing ? <div className="w-4 h-4 border-2 border-red-400/30 border-t-red-400 rounded-full animate-spin" /> : 'Close Ticket'}
              </Button>
            )}
          </div>
        </div>
      ...
      {/* Chat Messages */}
      <div className="flex-1 overflow-y-auto bg-[#0a0d14]/50 border border-white/[0.06] rounded-xl p-4 mb-4">
        <div className="space-y-4">
          {ticket.messages.map((msg) => {
            ...
            return (
              <div key={msg._id} className={`flex ${isAdminMsg ? 'justify-end' : 'justify-start'}`}>
                <div className="max-w-[75%]">
                  {/* Sender info */}
                  <div className={`flex items-center gap-1.5 mb-1 ${isAdminMsg ? 'justify-end' : ''}`}>
                    {isAdminMsg ? (
                    ...
                    <span className={`text-xs ${isAdminMsg ? 'text-purple-400' : isSeller ? 'text-emerald-400' : 'text-cyan-400'
                      }`}>
                    ...
                    {msg.read && <CheckCircle2 className="w-3 h-3 text-white/20" />}
                  </div>
                  {/* Bubble */}
                  <div className={`px-4 py-3 rounded-2xl ${isAdminMsg
                      ? 'bg-gradient-to-br from-purple-500/15 to-indigo-500/15 border border-purple-500/20 text-white'
                      : isSeller
                        ? 'bg-gradient-to-br from-emerald-500/10 to-green-500/10 border border-emerald-500/20 text-white'
      ...
      {/* Input Area */}
      {ticket.status !== 'closed' ? (
        <div className="shrink-0 bg-[#0f1117]/80 border border-white/[0.06] rounded-xl p-3">
          <div className="flex items-center gap-2 mb-2">
            <Shield className="w-4 h-4 text-purple-400" />
            <span className="text-purple-400 text-xs font-medium">Replying as Admin</span>
          </div>
          ...
          <div className="flex items-end gap-2">
            ...
            <textarea
              ...
              maxLength={2000}
              rows={1}
              className="flex-1 px-4 py-3 bg-white/[0.04] border border-white/[0.08] rounded-xl text-white placeholder:text-white/20 focus:outline-none focus:border-purple-500/40 resize-none text-sm"
            />
            <Button
              onClick={handleSend}
              disabled={sending || (!message.trim() && selectedFiles.length === 0)}
              className="bg-gradient-to-r from-purple-500 to-indigo-500 hover:from-purple-600 hover:to-indigo-600 text-white rounded-xl h-[46px] w-[46px] p-0 shrink-0"
            >
              {sending ? (
        ...
        <div className="shrink-0 bg-[#0f1117]/80 border border-white/[0.06] rounded-xl p-4 text-center">
          <p className="text-white/40 text-sm">
            Ticket closed {ticket.closedAt && `on ${new Date(ticket.closedAt).toLocaleString()}`}
            {ticket.closedBy && ` by ${ticket.closedBy.username}`}
          </p>
        </div>
      )}
    </div>
